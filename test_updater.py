...
class UpdaterTests(unittest.TestCase):
    def test_latest_release_requires_expected_assets_and_compares_tags(self) -> None:
        ...
        self.assertEqual(release.tag, "v0.10")
        self.assertTrue(updater.is_newer_release(release.tag, "v0.9"))
        self.assertFalse(updater.is_newer_release("v0.9", "v0.9"))
        self.assertTrue(updater.is_newer_release("v0.11"))
        self.assertFalse(updater.is_newer_release("v0.10"))
        self.assertFalse(updater.is_newer_release("release-candidate", "v0.9"))
    ...
    def test_verified_download_requires_release_checksum(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertEqual(staged.name, "StarEmpireCompanion-v0.10.update.exe")
    ...
    def test_replacement_helper_retries_a_locked_executable_and_records_failure(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            staged = Path(folder) / "StarEmpireCompanion-v0.10.update.exe"
